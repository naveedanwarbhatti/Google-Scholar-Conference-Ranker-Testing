...
function updateUI(response: ApiResponse): void {
  ...
  (document.getElementById('sca-spinner') as HTMLElement).style.display = 'none';
  (document.getElementById('sca-refresh') as HTMLButtonElement).style.display = 'block';
...
async function runAnalysis(): Promise<void> {
  ...
  const cache: { [key: string]: CacheEntry } = await chrome.storage.local.get(scholarId);
  ...
  const authorName = (document.getElementById('gsc_prf_in') as HTMLElement)?.innerText.trim();
  if (!authorName) {
  ...
  chrome.runtime.sendMessage(
    ...
    (response: ApiResponse) => {
      ...
      updateUI(response);
      if (response.status === 'success') {
        const cacheEntry: CacheEntry = { data: response, timestamp: Date.now() };
